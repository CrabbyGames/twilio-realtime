        ...
        async def send_to_twilio():
            ...
            try:
                async for openai_message in openai_ws:
                    ...
                    # print(f"--- Abnormal response: {response}")

                    if (
                        response.get("type") == "conversation.item.input_audio_transcription.completed" and
                        # and response["item"]["role"] == "user"\
                        response["transcript"]
                        # and response["item"]["content"][0]["type"] == "input_text"
                    ):
                        transcribed_text = response["transcript"]
                        current_transcript += "(me) > " + transcribed_text + "\n"
                    ...
                    if response.get("type") == "response.output_item.done":
                        transcribed_text = response.get('response', {}).get('output', [{}])[0].get('content', [{}])[0].get('transcript')
                        if transcribed_text:
                            current_transcript += "(caller) > " + transcribed_text + "\n"
